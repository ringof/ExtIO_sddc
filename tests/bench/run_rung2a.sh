#!/usr/bin/env bash
# tests/bench/run_rung2a.sh — Rung 2a: QDX CAT serial control test.
#
# Part of the local HITL bench plan (docs/local-hwil-plan.md). Rung 2a is the
# first QDX-touching rung: it proves the bench host can set frequency, key/
...
#
# Verdict line: "RUNG2A CAT OK ..." / "RUNG2A CAT FAIL — ...". Exit 0/1.

set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

fail() { echo "RUNG2A CAT FAIL — $*"; exit 1; }
...
# --- Run test --------------------------------------------------------------
exec python3 "$HERE/rung2a_cat_test.py"
