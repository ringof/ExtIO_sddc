...
/* ------------------------------------------------------------------ */
/* Modularization (issue #139): the shared lower layers and the          */
/* operator-diagnostics primitives now live in sibling modules.          */
/* fx3_core.{c,h} holds the diagnostics core (probe/poke/recover, upload, */
/* read-only status reads) shared with the rx888-tools diagnostics CLI;   */
...
/* The operator-diagnostics primitives (probe/poke/recover, firmware upload,
 * and the read-only status reads) now live in fx3_core.c.  What remains here
 * is the firmware regression / fuzz / soak harness plus the interactive debug
 * console and the command dispatcher. */
